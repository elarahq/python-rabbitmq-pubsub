...
class Publisher(object):
    ...
    def publish_message(self, message, routing_key):
        ...
        """
        print "In publish_message method"
        LOGGER.info("In publish_message method")
        if self._stopping:
            return

        # message = {u'key': u'value'}
...
def main():
    ...
    # host "/" (%2F)
    example = Publisher(
        'amqp://guest:guest@localhost:5672/%2F?connection_attempts=3&heartbeat_interval=3600', 'message')
    try:
        print "Before Run"
        example.run()
        print "After Run"
